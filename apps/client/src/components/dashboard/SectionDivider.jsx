...
export const SectionDivider = ({ label, className = '' }) => {
  return (
    <div className={`relative my-8 flex items-center justify-center ${className}`}>
      ...
      <div className="px-4 py-1 flex items-center gap-2 text-slate-400 dark:text-slate-500 font-mono text-xs uppercase tracking-[0.25em] font-semibold select-none">
        <span className="text-indigo-500/70 text-xs">◆</span>
        <span>{label}</span>
        <span className="text-indigo-500/70 text-xs">◆</span>
      </div>
